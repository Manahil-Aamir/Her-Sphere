...
class _PhotosState extends ConsumerState<Photos> {
  ...
  //Checking for permission of 'Photos'
    ...
    if (status != PermissionStatus.granted) {
      ...
      if (status == PermissionStatus.denied) {
        ...
        if (status == PermissionStatus.denied) {
          ...
          !(await _isAndroidVersionLessThan13())) {
  ...
  //Permission denied dialog for accessing gallery
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
              '$permissionName permissions are required to access the gallery.'),
          actions: <Widget>[
            ElevatedButton(
              onPressed: () {
  ...
  //Permission permanently denied dialog for accessing gallery
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
              'Please enable $permissionName permission in app settings to access the gallery.'),
          actions: <Widget>[
            ElevatedButton(
              onPressed: () {
                Navigator.of(context).pop();
  ...
  Future<void> uploadImageToStorage(
      File imageFile, String uid, FirebaseStorage storage) async {
    try {
      ...
      final storageRef = storage
          ...
          .child('user_images')
          .child(uid)
          .child(uniqueId); // Unique location for each image
  ...
  // Deleting an image from storage and Firestore
  Future<void> deleteImageFromStorageAndFirestore(
      String uid, String imageUrl, FirebaseStorage storage) async {
    try {
