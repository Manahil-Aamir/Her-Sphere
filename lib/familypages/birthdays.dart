...
Future<void> initializeNotifications() async {
            ...
            data: const CupertinoThemeData(
              textTheme: CupertinoTextThemeData(
                dateTimePickerTextStyle: TextStyle(
                  color: Color(0xFF726662),
                  fontSize: 20.0,
                  fontFamily: 'OtomanopeeOne', // or specify the fontFamily you desire
                ),
              ),
  ...
  if (name.isNotEmpty && selectedDate != null) {
    print('Birthday added: Name - $name, Date - $selectedDate  , notify: $notify');
    Navigator.of(context).pop();
    _checkPermissions();
      // Schedule notification if permission granted
    if (selectedDate != null) {
    _scheduleBirthdayNotifications(Birthday(name: name, date: selectedDate!));
    print('hiiiiii');
}
setState(() {
...
Future<void> _scheduleBirthdayNotifications(Birthday birthday) async {
  ...
  var birthdayThisYear = DateTime(today.year, birthday.date.month, birthday.date.day);

  // Check if the birthday has already passed this year
  if (today.isAfter(birthdayThisYear)) {
    // If it has passed, schedule the notification for next year
    birthdayThisYear = DateTime(today.year + 1, birthday.date.month, birthday.date.day);
  }
  ...
  var scheduledTimezoneDate = tz.TZDateTime.from(scheduledDate, tz.getLocation(timeZoneName));
  ...
  try {
    ...
    print('Notification scheduled successfully for ${birthday.name} at ${scheduledDate}');
...
// Schedule notifications only for the newly added birthday, 30 minutes before the birthday
Future<void> _futureBirthdays(Birthday birthday, DateTime scheduleDate) async {
  Future.delayed(const Duration(days: 365)); // Wait a year from now to check if the birthday still exists
  // Get today's date
  var today = DateTime.now();
  ...
  Future<void> _cancelBirthdayNotification(Birthday birthday) async {
    try {
      await flutterLocalNotificationsPlugin.cancel(birthdayList.indexOf(birthday));
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    int age = birthdayList[index].date.month > DateTime.now().month ||
                      (birthdayList[index].date.month == DateTime.now().month &&
                          birthdayList[index].date.day >= DateTime.now().day)
                      ? DateTime.now().year - birthdayList[index].date.year
                      : DateTime.now().year - birthdayList[index].date.year + 1;
                    
                    //Displaying how many days are left for birthday
                    int days = birthdayList[index].date.month > DateTime.now().month ||
                      (birthdayList[index].date.month == DateTime.now().month &&
                          birthdayList[index].date.day >= DateTime.now().day)
                      ? DateTime(DateTime.now().year, birthdayList[index].date.month, birthdayList[index].date.day)
                          .difference(DateTime.now()).inDays
                      : DateTime(DateTime.now().year + 1, birthdayList[index].date.month, birthdayList[index].date.day)
                          .difference(DateTime.now()).inDays;
                    ...
                    return Padding(
                      ...
                      child: ListTile(
                        ...
                        tileColor: Color((Random().nextDouble() * 0xFFFFFF).toInt()).withOpacity(0.3),
                        title: Text(
